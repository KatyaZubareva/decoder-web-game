...
const apiKey = process.env.CHAT_API_KEY || "dummy-key";
...
export function GameScreen() {
    ...
    useEffect(() => {
        ...
        const startChat = async() => {
            try {
                await client.connectUser({ id: "user-id", name: "User" }, "token");
    ...
    const UserAvatar = ({ src, name, team, isCaptain }) => (
        <div className="flex flex-col items-center gap-2 relative group">
            <div className={`w-14 h-14 rounded-full border-4 shadow-md overflow-hidden transition-transform transform group-hover:scale-105 ${team === 'blue' ? 'border-blue-400' : 'border-rose-400'}`}>
                <img src={src || `https://api.dicebear.com/7.x/avataaars/svg?seed=${name}`} alt={name} className="w-full h-full object-cover" />
            </div>
            {isCaptain && (
            ...
            <div className="flex-1 flex flex-col relative">
                ...
                <main className="flex-1 relative bg-slate-100/50 flex items-center justify-center p-6 overflow-hidden">
                    ...
                    <div className="absolute left-6 top-1/2 -translate-y-1/2 flex flex-col gap-4">
                        <UserAvatar name="Игрок A1" src="/assets/avatar3.png" team="blue" isCaptain={true} />
                        <UserAvatar name="Игрок A2" src="/assets/avatar4.png" team="blue" />
                    </div>
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col gap-4">
                        <UserAvatar name="Игрок B1" src="/assets/avatar1.png" team="red" isCaptain={true} />
                        <UserAvatar name="Игрок B2" src="/assets/avatar2.png" team="red" />
                    </div>
            ...
            <div className="w-80 lg:flex flex-col border-l border-slate-200 shadow-xl hidden">
                ...
                <div className="flex-1 flex flex-col bg-white">
                    ...
                    <div className="flex border-b border-slate-100 p-1">
                        ...
                        <button onClick={() => setIsOpen(true)} className="p-3 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg">
                            <Settings size={20} />
                        </button>
                    </div>
